...
from nets.stdcnet import STDCNet1446, STDCNet813, STDCNetSpitial
...
class SpatialPath(nn.Module):
    def __init__(self, backbone='Spatial'):
        super(SpatialPath, self).__init__()

        self.backbone_name = backbone
        if backbone == 'Spatial':
            self.backbone = STDCNetSpitial()
            # self.arm16 = AttentionRefinementModule(512, 128)
            # inplanes = 1024
            # if use_conv_last:
            #     inplanes = 1024
...
class BiSeNet(nn.Module):
    def __init__(self, backbone, n_classes, pretrain_model='', use_boundary_2=False, use_boundary_4=False, use_boundary_8=False, use_boundary_16=False, use_conv_last=False, heat_map=False, *args, **kwargs):
        ...
        self.spitial_backbone = SpatialPath('Spatial')
        ...
        self.S_ffm = FeatureFusionModule(512, 256)
    ...
    def forward(self, x):
        H, W = x.size()[2:]         # [4,3,1024,2048]

        # x_context = F.interpolate(x,scale_factor=0.75)
        # x_context = F.interpolate(x,(int(H*1.5),int(W*1.5)))
        x_context = x
        feat_res2, feat_res4, feat_res8, feat_res16, feat_cp8, feat_cp16 = self.cp(x_context)
        ...
        # spatial
        # x_spial = F.interpolate(x, (2*H, 2*W), mode='nearest')
        x_spial = F.interpolate(x,scale_factor=0.5)
        # x_spial = x
        feat_spial = self.spitial_backbone(x_spial)[2]
        F_H,F_W = feat_res8.size()[2:]
        feat_spial = F.interpolate(feat_spial,(F_H,F_W))

        # # # check FFM'S features shape equal
        # print(feat_spial.shape)
        # print(feat_res8.shape)

        feat_sffm = self.S_ffm(feat_spial,feat_res8)

        feat_fuse = self.ffm(feat_sffm, feat_cp8)
...
if __name__ == "__main__":
    ...
    net = BiSeNet('STDCNet813', 6)
    net.cuda()
    net.eval()
    in_ten = torch.randn(1, 1, 448, 704).cuda()
    out, out16, out32 = net(in_ten)
    # wd,nowd,lr,lr_now = net.get_params()
    print(out.shape)

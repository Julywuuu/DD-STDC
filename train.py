...
def parse_args():
    ...
    parse.add_argument( '--n_img_per_gpu',dest = 'n_img_per_gpu',type = int,default = 2, )
    parse.add_argument('--max_iter',dest = 'max_iter', type = int,default = 10000,)
    parse.add_argument( '--save_iter_sep',dest = 'save_iter_sep', type = int, default = 50,)
    ...
    parse.add_argument( '--respath',   dest = 'respath',type = str, default = './checkpoints/camera_4',)
...
def train():
    ...
    for it in range(max_iter):
        ...
        #     print(im.shape)
        #     print(lb.shape)
        #     check_img = np.array(im[0].permute(1,2,0).detach().cpu(),dtype=np.uint8)
        #     check_lb = np.array(lb[0].detach().cpu(), dtype=np.uint8)
        #
        #     cv2.imshow('check_img',check_img)
        #     cv2.imshow('check_lb',check_lb)
        #     cv2.waitKey(0)
        ...
        if (it+1)%save_iter_sep==0:# and it != 0:
            ...
            .format(it+1, str(round(mIOU50,4)), str(round(mIOU75,4))))                      # ound() save 4wei xiaoshu
